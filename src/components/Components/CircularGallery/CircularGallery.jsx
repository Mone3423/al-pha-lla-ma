...
class VideoMedia extends Media {
  constructor({ source, poster, autoplay = true, loop = true, muted = true, ...rest }) {
    super({ ...rest, image: poster }); // Usar poster como imagen inicial
    ...
    // Actualizar textura cuando el video está listo
    this.videoElement.addEventListener('playing', () => {
      this.program.uniforms.tMap.value = this.videoTexture;
    });
  }
...
class App {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    ...
    const galleryItems = items && items.length ? 
      items.map(item => ({ image: item.image || item.src })) : // Acepta tanto 'image' como 'src'
      defaultItems;
